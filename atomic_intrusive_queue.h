...
namespace exec {
template <auto _NextPtr> class __atomic_intrusive_queue;
...
public:
  ...
  auto try_push_front(__node_pointer t) noexcept -> try_push_result {
    ...
    return {__head_.compare_exchange_strong(__old_head, t,
  ...
  auto push_front(__node_pointer t) noexcept -> bool {
    ...
    do {
      t->*_NextPtr = __old_head;
    } while (!__head_.compare_exchange_weak(__old_head, t,
                                            std::memory_order_acq_rel));
  ...
  void prepend(stdexec::__intrusive_queue<_NextPtr> queue) noexcept {
    ...
    while (!__head_.compare_exchange_weak(__old_head, __new_head,
                                          std::memory_order_acq_rel)) {
      __tail->*_NextPtr = __old_head;
    }
    queue.clear();
...
private:
  auto reset_head() noexcept -> __node_pointer {
    __node_pointer __old_head = __head_.load(std::memory_order_relaxed);
    while (!__head_.compare_exchange_weak(__old_head, nullptr,
                                          std::memory_order_acq_rel)) {
      ;
    }
    return __old_head;
