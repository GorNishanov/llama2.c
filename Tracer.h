...
class global_trace_dumper {
  ...
  static timepoint get_seq();
  static void log(timepoint seq, std::size_t size, std::string_view, std::string_view, std::format_args);
...
void log_info_with_prefix(std::string_view prefix, std::format_string<Args...> fmt, Args &&...args) {
  auto seq = global_trace_dumper::get_seq();
