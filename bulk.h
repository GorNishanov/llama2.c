...
{
struct fast_clock { // wraps GetSystemTimeAsFileTime
  using rep = long long;
...
#pragma warning(disable : 4324) // structure was padded
    ...
    {
        std::println("{:x} {} {}", GetCurrentThreadId(), no, std::vformat(fmt.get(), std::make_format_args(args...)));
    }
#else
    template <class... Args>
    ...
    void end_batch(std::chrono::fast_clock::time_point started)
    {
...
namespace std {
template <typename F> void bulk_schedule_second(unsigned N, F f) {
